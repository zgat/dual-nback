...
主页由一份稳定的 `GameHome` 渲染，三个状态 Hook 分别管理 N-Back、翻牌记忆和反应力测试，游戏组件只负责展示。切换玩法不重新创建主页，展开的设置区域按内容高度连续过渡。

动画统一使用短时变换和透明度过渡；弹窗退场期间仍阻止游戏输入并暂停计时。倒计时遮罩立即出现、扑克完整可见时长不含翻牌动画、反应目标即时显示。系统开启“减少动态效果”时简化装饰动画，保留能看清的移牌轨迹。

游戏到结果页、历史最佳分类及偏好设置到捐赠页共用 `TransitionSurface`：旧视图短暂保留但不可交互，计时和状态仍由外层 Hook 管理。阶段按钮与文案节点持续复用，快速切换可反向衔接。扑克计时模式先盖牌再换牌；翻牌记忆的开牌、盖牌、换牌阶段显式锁定输入，装饰性发牌时间不计入成绩，错误牌仍在 650 毫秒有效时间后盖回。
